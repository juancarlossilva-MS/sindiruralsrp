...
function createData(titulo,id,pastaImgClass) {
  return { titulo,id,pastaImgClass };
}
...
function DelModal(){
  ...
  return(
    <Dialog
      ...
      >
       <DialogTitle id="alert-dialog-title">Insira sua senha para deletar o galeria: {selClass.titulo}</DialogTitle>
...
function confirmaDel(){
    ...
    .then((userCredential) => {
      ...
      fire.database().ref("/galeria/"+selClass.id).remove()
...
useEffect(() =>{
    ...
    setRows([]);
    var lc = fire.database().ref('galeria');
      

        lc.on("value",(snap) => {
            snap.forEach((c) => {
                  var nc = c.val();
                  setRows(prev=>[...prev,createData(nc.titulo,c.key,nc.pastaImgClass)]);
            });
        });
  ...
  return (
      <>
        <main  className={classes.content}>
          <Link href="/admin/addGaleria" >
            <Button style={{backgroundColor:"#023723",float:"right"}} round>
                <Add className={classes.icons} /> Galeria
              </Button>
          </Link>
