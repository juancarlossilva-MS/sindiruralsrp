...
function DelModal(){
  ...
  return(
    <Dialog
      ...
      >
       <DialogTitle id="alert-dialog-title">Insira sua senha para deletar a Curso: {selNews.nome}</DialogTitle>
        <DialogContent>
...
function confirmaDel(){
  ...
  .then((userCredential) => {
    ...
    fire.database().ref("/cursos/"+selNews.id).remove().then(()=>{
        
      fire.storage().ref().child('cursos/'+selNews.imagem).delete().then(function() {
...
React.useEffect(() =>{
    setRows([]);
    var lc = fire.database().ref('cursos');
  ...
  function editCurso(id){
    setOpen(true);
    router.push({pathname: '/admin/editCurso',query: { id: id }})
  ...
  return (
      <>
        <main  className={classes.content}>
          <Link href="/admin/addCurso" >
            <Button style={{backgroundColor:"#023723",float:"right"}} round>
                <Add className={classes.icons} /> Curso
              </Button>
          </Link>
        ...
        <TableContainer className={classes.container}>
            ...
            <TableBody>
                ...
                return (
                    ...
                    <TableCell>
                        <Button onClick={()=>editCurso(row.id)}>
                            <Edit/></Button>
